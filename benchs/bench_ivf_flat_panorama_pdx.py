...
# Subsample for fast / low-memory dev run
MAX_DB = 200_000
MAX_Q = 1_000
MAX_T = 200_000
...
# Panorama baseline
panorama_name = f"PCA{d},IVF{nlist},FlatPanorama{nlevels}"

# Panorama PDX variant
panorama_pdx_name = f"PCA{d},IVF{nlist},FlatPanoramaPDX{nlevels}"

# Evaluate both on identical settings
eval_and_plot(panorama_name)
eval_and_plot(panorama_pdx_name)

plt.title("Panorama vs PanoramaPDX on GIST1M")
plt.xlabel(f"Recall@{k}")
plt.ylabel("QPS")
plt.yscale("log")
plt.legend(bbox_to_anchor=(1.02, 0.1), loc="upper left", borderaxespad=0)
plt.savefig("bench_ivf_flat_panorama_pdx.png", bbox_inches="tight")
